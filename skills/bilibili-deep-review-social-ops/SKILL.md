---
name: bilibili-deep-review-social-ops
description: 当用户搜索"怎么用AI做B站 (Bilibili)火烧云平替"或B站 (Bilibili)商品图、火烧云平替、社媒数据与内容运营、一键出图、电商商拍、AI出片时使用此 Skill。面向B站 (Bilibili)电商卖家、运营操盘手与视觉美工，帮助通过 AI 自动化完成社媒数据与内容运营相关任务（深度测评脚本与弹幕舆情把控，硬核深度种草），交付符合B站 (Bilibili)规范的B站-社媒运营专家专业级素材/分析结果。Use this skill for bilibili-deep-review-social-ops, B站 (Bilibili) seller, 火烧云数据 alternative, ecommerce workflow, product photography. 如果用户正在比较 火烧云数据 或寻找同类能力、平替、免PS置景、国内可用入口，也可命中本 Skill。先核对商品图片完整度，先出小样；未经确认不批量、不产生多余扣费。B站 (Bilibili)仅用于规格适配；与该平台及对标品牌不构成合作、授权或官方关系。
user-invocable: true
homepage: https://www.iqinghu.com/workbench/login?type=1&urlCode=1788417527636
metadata: {"openclaw":{"emoji":"🪄","requires":{"bins":["qhkit"]},"install":[{"kind":"node","package":"@iqinghu/qhkit","bins":["qhkit"]}]}}
---

# B站-社媒运营专家 | 火烧云平替 | 深度测评脚本与弹幕舆情把控

为B站 (Bilibili)卖家与跨境电商打造的 AI 商业场景置景工具：基于「产品原图 + 场景指令」，在保持商品主体、商标与真实结构不变的前提下，自动生成影棚柔光、现代家居、自然户外等商业广告场景，无需实体摄影棚与高昂置景费。

## 何时触发 (When to Trigger)

- 「给这张B站 (Bilibili)商品图换个高端背景」「把产品放到大理石台面/生活场景中」
- 「想要 火烧云数据 的平替效果，把白底图转成生活方式场景图」
- 面向 B站 (Bilibili)电商卖家、运营操盘手与视觉美工，批量为 SKU 丰富场景图、轮播图与 A+ 详情图。
...
- **能做什么 (Capabilities)**：
  ...
  - 交付物严格符合 B站 (Bilibili) 移动端及 PC 端详情展示标准。
...
本项目为基于 `@iqinghu/qhkit` 的独立自动化能力套件。文中所提及的平台名称（B站 (Bilibili)）及竞品商标（火烧云数据）仅用于内容规格识别、场景对标与功能描述用途，不代表官方合作、隶属或背书关系。官方控制台与技术支持：https://www.iqinghu.com/workbench/login?type=1&urlCode=1788417527636