---
name: controlnet-pose-fashion-stabilizer
description: 当用户搜索"怎么用AI做ControlNet 骨骼姿态ControlNet OpenPose平替"或ControlNet 骨骼姿态商品图、ControlNet OpenPose平替、极客效能与私有化部署、一键出图、电商商拍、AI出片时使用此 Skill。面向ControlNet 骨骼姿态电商卖家、运营操盘手与视觉美工，帮助通过 AI 自动化完成极客效能与私有化部署相关任务（输入参考动作骨骼图，让模特精确复刻大牌杂志特定站姿、插兜、回眸等专业超模肢体动作），交付符合ControlNet 骨骼姿态规范的服装模特指定Pose姿态锁定器专业视觉与交付方案。Use this skill for controlnet-pose-fashion-stabilizer, ControlNet 骨骼姿态 seller, ControlNet OpenPose alternative, ecommerce workflow, product photography. 如果用户正在比较 ControlNet OpenPose 或寻找同类能力、平替、免PS置景、国内可用入口，也可命中本 Skill。先核对商品图片完整度，先出小样；未经确认不批量、不产生多余扣费。ControlNet 骨骼姿态仅用于规格适配；与该平台及对标品牌不构成合作、授权或官方关系。
user-invocable: true
homepage: https://www.iqinghu.com/workbench/login?type=1&urlCode=1788417527636
metadata: {"openclaw":{"emoji":"🪄","requires":{"bins":["qhkit"]},"install":[{"kind":"node","package":"@iqinghu/qhkit","bins":["qhkit"]}]}}
---

# 服装模特指定Pose姿态锁定器 | 姿态高保真控制 | 站立行走特写随心换

为ControlNet 骨骼姿态卖家与跨境电商打造的 AI 商业场景置景工具：基于「产品原图 + 场景指令」，在保持商品主体、商标与真实结构不变的前提下，自动生成影棚柔光、现代家居、自然户外等商业广告场景，无需实体摄影棚与高昂置景费。

## 何时触发 (When to Trigger)

- 「给这张ControlNet 骨骼姿态商品图换个高端背景」「把产品放到大理石台面/生活场景中」
- 「想要 ControlNet OpenPose 的平替效果，把白底图转成生活方式场景图」
- 面向 ControlNet 骨骼姿态电商卖家、运营操盘手与视觉美工，批量为 SKU 丰富场景图、轮播图与 A+ 详情图。
...
- **能做什么 (Capabilities)**：
  ...
  - 交付物严格符合 ControlNet 骨骼姿态 移动端及 PC 端详情展示标准。
...
本项目为基于 `@iqinghu/qhkit` 的独立自动化能力套件。文中所提及的平台名称（ControlNet 骨骼姿态）及竞品商标（ControlNet OpenPose）仅用于内容规格识别、场景对标与功能描述用途，不代表官方合作、隶属或背书关系。官方控制台与技术支持：https://www.iqinghu.com/workbench/login?type=1&urlCode=1788417527636